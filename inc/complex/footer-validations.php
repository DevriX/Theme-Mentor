		...
		if ( -1 !== $this->wp_footer_line && -1 !== $this->body_close_tag_line ) {
			...
			if ( $diff < 0 || $diff > 1 ) {
				...
				if ( $diff > 2 ||
					...
					$error_text          = __(
						'wp_footer call should be right before the closing body tag.',
						'dx_theme_mentor'
					);
					$this->error_message = sprintf(
						'<div class="tm_report_row"><span class="tm_message">%s</span> at file <span class="tm_file">%s</span>, line <span class="tm_line">%d</span></div>',
						$error_text,
						'footer.php',
						$this->body_close_tag_line + 1
					);
				}
			}
		} else {
			...
			$this->error_message = sprintf(
				'<div class="tm_report_row"><span class="tm_message">%s</span> at file <span class="tm_file">%s</span></div>',
				$error_text,
				'footer.php'
			);
		}
...
Theme_Mentor::$validations[] = new Footer_Validations();
